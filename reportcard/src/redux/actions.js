  ...
  (updatedStudent, selectedClass,selectedSection) => async (dispatch) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3001/studentdata/${updatedStudent.adm_no}`,
...
export const fetchStudentMarks = (selectedClass, selectedSubject,selectedSection) => {
  return async (dispatch) => {
    try {
      const response = await axios.post("http://localhost:3001/student/marks", {
        selectedClass,
        selectedSubject,
        selectedSection,
      });
...
export const updateStudentMarks =
  ...
  async (dispatch) => {
    try {
      const response = await axios.put(
        `http://localhost:3001/student/marks/${adm_no}`,
...
export const fetchTeachers = () => {
  return async (dispatch) => {
    try {
      const response = await axios.get("http://localhost:3001/staff/teachers");
...
export const fetchClasses = () => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get("http://localhost:3001/staff/classes");
...
export const updateClasses = (selectedTeacher, selectedClass, selectedSubjects,selectedSection) => async (dispatch) => {
  try {
    const response = await axios.post(
      `http://localhost:3001/staff/updatedata`,
...
export const removeClass = (selectedTeacher, selectedClass, subject) => async (dispatch) => {
  try {
    await axios.post(`http://localhost:3001/staff/removedata`, {
      selectedTeacher,
      selectedClass,
      subject,
    });
