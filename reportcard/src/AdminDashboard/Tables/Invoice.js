...
const Invoice = () => {
  ...
  const fetchTeachers = async () => {
    try {
      const response = await fetch("http://localhost:3001/staff/teachers");
  ...
  const fetchSalaries = async () => {
    try {
      const response = await fetch("http://localhost:3001/staff/salaries");
      const result = await response.json();
      setSalaries(result);
  ...
  // console.log(salaries[0]?.payment_date.substring(0,4))
  return (
