...
import { fetchStudentData, fetchClasses } from "../redux/actions";
const BioData = () => {
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const response = await axios.post(
          "http://localhost:3001/admin/bio-data",
          values
        );
  ...
  useEffect(() => {
    ...
    console.log(formik.errors.class_name);
    console.log(formik.errors.section);
  ...
  const handleFile = async (e) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/admin/upload/student-data",
        formData,
        {
          headers: {
