...
export default function CreateClass() {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        const response = await axios.get("http://localhost:3001/cls/subjects");
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        const response = await axios.post(
          "http://localhost:3001/cls/create-class",
          values
        );
  ...
  const handleNewSubjectSubmit = async () => {
    try {
      if (!subjects.includes(newSubject)) {
        ...
        const response = await axios.post(
          "http://localhost:3001/cls/create-newsubject",
          { newSubject }
        );
  ...
  const handleDelSubjectSubmit = async () => {
    try {
      if (subjects.includes(newSubject)) {
        ...
        const response = await axios.post(
          "http://localhost:3001/cls/delete-subject",
          { newSubject }
        );
        console.log(response.data);
